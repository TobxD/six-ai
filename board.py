...
class Board:
    ...
    def hasSpecificWinningShape(self, y, x, shape):
        ...
        for y_dif, x_dif in shape:
            ...
            if (
                not self.inBounds(new_y, new_x)
                or self.board[y + y_dif][x + x_dif] != color
            ):
                return 0
        return color
    ...
    def hasWon(self):
        ...
        for player in range(2):
            ...
            valid_mask = (
                (pattern_positions >= 0) & (pattern_positions < self.size)
            ).all(axis=(-2, -1))
            valid_pattern_positions = pattern_positions[valid_mask]
            pattern_res = self.stones[player][
                valid_pattern_positions[..., 0], valid_pattern_positions[..., 1]
            ]
